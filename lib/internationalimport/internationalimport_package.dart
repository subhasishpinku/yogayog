...
class InternationalImportPackage extends StatefulWidget {
  const InternationalImportPackage({
    super.key,
    this.type = 'Document',
    this.size = '0 - 500g',
    this.pieces = '1',
  ...
  @override
  State<InternationalImportPackage> createState() =>
...
class _InternationalDetailsPackageState
    extends State<InternationalImportPackage> {
  late String type = widget.values['packageType']?.toString() ?? widget.type;
  late String size = widget.values['packageSize']?.toString() ?? widget.size;
  late final pieces = TextEditingController(
    text: widget.values['pieces']?.toString() ?? widget.pieces,
  );
  late final weight = TextEditingController(
    text: widget.values['weight']?.toString() ?? widget.weight,
  );
  final boxes = <_Box>[];
  ...
  void initState() {
    super.initState();
    final initialBoxes = widget.values['boxes'] is List
        ? (widget.values['boxes'] as List).whereType<Map>()
        : widget.boxes;
    for (final item in initialBoxes) {
      final box = _Box();
